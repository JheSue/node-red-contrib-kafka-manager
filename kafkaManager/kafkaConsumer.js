...
/*
	const addTopics=change.add.filter(cell=>{
		const match=node.filters.find(regex=>{
			const found=regex.test(cell.topic)
			console.log("match regs: \""+cell.topic+"\" regex: "+regex+" result:  "+found+" retry "+regex.test(cell.topic)+" retry "+regex.test(cell.topic))
			return found
		})
//		const match=node.filters.find(regex=>regex.test(cell.topic))
		console.log("match: \""+cell.topic+"\" found: "+(match!==undefined)+" result:  "+match)
